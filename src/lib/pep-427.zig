...
pub const ProjectName = struct {
    ...
    pub fn parse(allocator: std.mem.Allocator, name: []const u8) !ProjectName {
...
test "noop" {
    const project_name = try ProjectName.parse(std.testing.allocator, "cowsay");
...
test "lowercase" {
    const project_name = try ProjectName.parse(std.testing.allocator, "PyYAML");
    defer project_name.deinit();
...
test "special chars" {
    const project_name = try ProjectName.parse(std.testing.allocator, "twitter.commons.lang");
    defer project_name.deinit();
...
test "runs of special chars" {
    const project_name = try ProjectName.parse(std.testing.allocator, "foo-_.bar_baz");
    defer project_name.deinit();
